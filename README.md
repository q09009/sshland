...
> [!IMPORTANT]
> 최신 공개 버전은 `v0.1.1` 프리릴리스이며 설치 파일은 [GitHub Releases](https://github.com/q09009/sshland/releases/tag/v0.1.1)에서 받을 수 있습니다. 중요한 서버에서 사용하기 전에는 [보안 및 주의사항](#보안-및-주의사항)을 확인해주세요.
...
- 같은 화면에서 전환하거나 나란히 열 수 있는 **내 PC·서버 공용 파일관리자**
- 파일과 폴더의 재귀 업로드·다운로드, 서버 내 복사, 이동, 이름 변경, 삭제
- 드래그 영역·`Ctrl`·`Shift`를 이용한 다중 선택과 선택 항목 일괄 작업
- 현재 폴더 즉시 필터와 서버의 `fd`·`find`를 선택하는 재귀 검색
...
- TOML 파일로 확장할 수 있는 터미널 명령 GUI와 대시보드 위젯
...
| macOS | 아직 검증하지 않았습니다. |
| 원격 서버 | Linux 계열 SSH 서버를 기준으로 합니다. 기본 대시보드와 터미널 명령 GUI는 `sh`, `df`, `ps` 같은 일반적인 Linux 명령을 사용합니다. |
...
- 상단의 **내 PC**와 **서버**를 눌러 로컬 파일과 원격 파일을 전환합니다. 각 파일관리자 pane은 마지막으로 열었던 로컬·원격 경로를 따로 기억합니다.
- 화면을 분할해 한쪽은 **내 PC**, 다른 쪽은 **서버**로 열면 파일과 폴더를 두 pane 사이에서 바로 끌어 전송할 수 있습니다. PC↔서버 드래그는 안전하게 **복사**로 동작해 원본이 남고, 같은 쪽의 다른 폴더로 끄는 동작은 **이동**입니다.
- PC↔서버 전송 대상에 같은 이름이 있으면 `복사본`, `복사본 2`처럼 새 이름을 만들어 기존 항목을 보존합니다. 전송 진행 상태와 실패 내역은 기존 업로드·다운로드 표시줄에서 확인합니다.
- 폴더를 더블 클릭하면 해당 폴더로 이동합니다.
- 서버의 편집 가능한 파일을 더블 클릭하면 코드 편집기 pane에서 엽니다. 현재 내 PC 파일은 앱 안에서 편집하지 않으며 탐색과 전송·파일 작업에 사용합니다.
- 빈 공간에서 마우스 왼쪽 버튼을 누른 채 드래그하면 사각형 안의 파일과 폴더를 여러 개 선택합니다.
- `Ctrl`을 누른 채 클릭하면 항목을 하나씩 추가·해제하고, `Shift`를 누른 채 클릭하면 마지막 선택 지점까지 범위로 선택합니다. macOS에서는 `Ctrl` 대신 `Command`를 사용합니다.
- 선택한 여러 항목은 한 번에 다운로드·복사·이동·삭제할 수 있습니다. 선택 항목 중 하나를 다른 폴더나 다른 파일관리자 pane으로 끌면 함께 처리됩니다.
- 파일이나 빈 공간을 마우스 오른쪽 버튼으로 눌러 새 파일·폴더, 다운로드, 복사, 이름 변경, 삭제 메뉴를 사용할 수 있습니다. 이름 변경은 한 항목을 선택했을 때만 사용할 수 있습니다.
- 로컬 파일이나 폴더를 파일관리자 위로 끌어놓으면 현재 원격 폴더에 업로드합니다. 폴더의 하위 구조와 빈 폴더도 그대로 생성됩니다.
- 원격 폴더를 다운로드하면 하위 파일과 빈 폴더까지 선택한 로컬 폴더 아래에 그대로 생성됩니다.
- 서버 검색 방식은 설정의 **파일 관리자**에서 선택할 수 있습니다. **내 PC**에서는 별도 명령을 실행하지 않고 현재 폴더 빠른 필터를 사용합니다.
  - **현재 폴더 빠른 필터**: 이미 불러온 현재 폴더의 이름을 입력과 동시에 거르며 서버 명령을 실행하지 않습니다.
  - **fd 재귀 검색**: `Enter`를 누르면 현재 폴더 아래를 `fd` 또는 `fdfind`로 검색합니다. 서버에 별도 설치가 필요합니다.
  - **find 재귀 검색**: `Enter`를 누르면 현재 폴더 아래를 일반적인 `find` 명령으로 검색합니다.
- 접속 과정에서는 검색 명령을 확인하지 않습니다. 설정에서 **fd** 또는 **find** 옵션을 누를 때 해당 명령만 확인하며, 사용할 수 있을 때 선택이 적용됩니다. 명령이 없으면 기존 선택을 유지합니다.
- 확인 결과는 서버 주소·포트·SSH 사용자별로 설정 파일에 저장됩니다. 같은 서버에 다시 접속하면 저장된 결과를 바로 사용하며, 옵션을 다시 누르면 현재 설치 상태를 재확인합니다.
- `fd`·`find` 검색을 실행하면 사용한 검색 명령이 화면 아래 명령어 로그에도 기록됩니다.
- 재귀 검색 결과는 경로와 함께 최대 500개까지 표시되며, 일반 목록처럼 열거나 여러 개를 선택해 파일 작업을 할 수 있습니다. 숨김파일 표시가 꺼져 있으면 숨김 폴더도 탐색하지 않습니다.
- 상단의 **보기** 메뉴에서 목록, 자세히, 큰 아이콘 보기와 숨김파일 표시를 변경할 수 있습니다.

| 파일관리자 기능 | 단축키 |
| --- | --- |
| 현재 폴더 검색창 열기 | `Ctrl+F` / `Command+F` |
| 화면에 보이는 항목 모두 선택 | `Ctrl+A` / `Command+A` |
| 선택 해제 | `Esc` |

> [!CAUTION]
> 탐색기 pane 사이의 PC↔서버 드래그·붙여넣기는 이름 충돌 시 복사본 이름을 사용하지만, 운영체제 탐색기에서 서버 pane으로 직접 끌어놓는 기존 업로드는 같은 이름의 원격 파일을 별도 확인 없이 덮어씁니다. 같은 이름의 원격 폴더는 병합되며 그 안의 같은 이름 파일도 덮어씁니다. 기존 **다운로드** 메뉴도 선택한 로컬 위치에 따라 병합·덮어쓰기될 수 있습니다. 또한 내 PC에서 삭제한 항목은 운영체제 휴지통을 거치지 않습니다. 심볼릭 링크는 플랫폼별 동작과 경로 탈출을 피하기 위해 폴더 재귀 전송에서 지원하지 않습니다. 중요한 경로에서 작업하기 전에는 백업을 권장합니다.
...
- 설정에서 파일 검색 방식, 언어, 터미널 명령 GUI, 대시보드, 시계 표시와 테마를 조정할 수 있습니다. 앱 전체의 하단 명령어 로그 설정은 **일반**에 있습니다.
...
`settings.json`은 공유 대상이 아닙니다. 따라서 마지막 서버 주소, 사용자명, 개인키 경로, 서버별 검색 명령 확인 결과, 대시보드 구성 같은 개인 앱 설정은 테마에 포함되지 않습니다. 비밀번호와 개인키 암호는 원래부터 `settings.json`에도 저장하지 않습니다.
...
├─ src-tauri/default_commands/  # 기본 터미널 명령 GUI 정의
