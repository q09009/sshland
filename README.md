...
- 선택한 여러 항목은 한 번에 다운로드·복사·이동·삭제할 수 있습니다. 선택 항목 중 하나를 다른 폴더나 다른 파일관리자 pane으로 끌면 함께 이동합니다.
...
- 검색 방식은 설정의 **파일 관리자**에서 선택할 수 있습니다.
...
> [!CAUTION]
> 업로드 대상에 같은 이름의 원격 파일이 있으면 별도 확인 없이 덮어씁니다. 같은 이름의 원격 폴더는 병합되며, 그 안의 같은 이름 파일도 덮어씁니다. 여러 항목이나 폴더를 다운로드할 때 선택한 로컬 위치에 같은 이름이 있으면 역시 병합·덮어쓰기될 수 있습니다. 심볼릭 링크는 플랫폼별 동작과 경로 탈출을 피하기 위해 폴더 재귀 전송에서 지원하지 않습니다. 중요한 경로에서 전송하기 전에는 백업을 권장합니다.
