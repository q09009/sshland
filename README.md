...
- 체크박스로 설정하는 서버 파일·폴더 Unix 권한과 선택적 하위 항목 일괄 적용
...
- 파일이나 빈 공간을 마우스 오른쪽 버튼으로 눌러 새 파일·폴더, 다운로드, 복사, 이름 변경, 권한 변경, 삭제 메뉴를 사용할 수 있습니다. 이름 변경과 권한 변경은 한 항목을 선택했을 때만 사용할 수 있습니다.
- 서버의 파일이나 폴더에서 **권한 변경**을 열면 소유자·그룹·기타 사용자의 읽기·쓰기·실행 권한을 체크박스로 설정할 수 있습니다. 폴더는 선택적으로 내부 모든 항목에 같은 권한을 적용하며, 심볼릭 링크는 따라가지 않습니다. 로컬 파일은 운영체제마다 권한 모델이 달라 이번 메뉴에서 다루지 않으며, 권한 변경은 **서버** 화면에만 표시됩니다.
...
> [!WARNING]
> 폴더 권한을 하위 항목에 일괄 적용할 때 실행 권한을 제거하면 해당 폴더 안으로 들어갈 수 없게 될 수 있습니다. 현재 사용자에게 변경 권한이 있는 서버 항목에만 적용됩니다.
